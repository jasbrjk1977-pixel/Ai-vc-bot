...
async function handleImagine(interaction) {
  ...
  if (!HF_TOKEN) {
    return interaction.editReply('❌ `HF_TOKEN` is not set. Please add your Hugging Face token to Railway variables.');
  }

  try {
    await interaction.editReply('⏳ Generating your image, please wait...');

    // Use the @huggingface/inference SDK — more reliable on Railway than raw fetch
    ...
    console.log(`🎨 /imagine generated for: "${fullPrompt}"`);
  } catch (err) {
    console.error('❌ /imagine failed:', err.message);
    // 503 = model still loading on HuggingFace free tier
    if (err.message && err.message.includes('503')) {
      return interaction.editReply('⏳ The image model is warming up. Please try again in **30 seconds**.');
    }
