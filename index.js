...
async function handleImagine(interaction) {
  ...
  // Build the final prompt (append style if provided)
  const fullPrompt = style ? `${userPrompt}, ${style} style` : userPrompt;

  // URL-encode and build a Pollinations.ai URL (free, no API key needed)
  const encodedPrompt = encodeURIComponent(fullPrompt);
  const seed          = Math.floor(Math.random() * 1_000_000);
  const imageUrl      = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1024&height=1024&seed=${seed}&nologo=true`;

  try {
    // Fetch the image as a buffer so we can attach it directly to Discord
    ...
    await interaction.editReply(`❌ Sorry, I couldn't generate that image. Please try a different description.\n*(Error: ${err.message})*`);
  }
}
