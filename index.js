...
// ─── /imagine (FreeTheAI — free, gpt-image-2) ───────────────────────────────
async function handleImagine(interaction) {
  ...
  const FREETHEAI_KEY = process.env.FREETHEAI_KEY;
  if (!FREETHEAI_KEY) {
    return interaction.editReply('❌ `FREETHEAI_KEY` is not set in Railway variables.');
  }

  try {
    await interaction.editReply('🎨 Generating your image, please wait...');

    const response = await fetch('https://api.freetheai.xyz/v1/images/generations', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${FREETHEAI_KEY}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 && errData?.error?.type === 'daily_checkin_required') {
        return interaction.editReply('⚠️ The bot needs a daily check-in. Ask the bot owner to run `/checkin` in the FreeTheAI Discord server.');
      }
    ...
    if (item?.b64_json) {
      ...
      if (!imgRes.ok) throw new Error('Could not download image URL');
      imageBuffer = Buffer.from(await imgRes.arrayBuffer());
    } else {
      throw new Error('No image returned from FreeTheAI');
    }
